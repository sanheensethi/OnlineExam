...
/*********** EXAM GIVER CHECK VIEW PAGE *************/
	...
	if($_POST['page']=='examgiver'){
		...
		//var_dump($_POST);
			...
			<button type='button' name='next' class='btn btn-danger' onclick='bookmark(".$quesSHOWid.")' id='bookmark'>Bookmark and Next</button><br><br>
			...
			<script>
				...
				foreach($qid_array as $qid){
					...
					$showQDemo .= '+ "<li class='.$class.'><a href=\'javascript:void(0)\' '.$style.' '.$style2.' onclick=\'callQ('.($i).')\'>'.($i+1)." QID ".$qid.'</a></li>"';
				$i++;
				}
				...
				$showQDemo .= ' + "</ul>" ); demo.click(function () {
