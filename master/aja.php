...
if(isset($_POST['page']))
{
	...
	if($_POST['page'] == 'exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			{
				$exam->query = $exam->query.' online_exam_title LIKE "%'.$_POST["search"]["value"].'%" ';

				$exam->query = $exam->query.' OR online_exam_datetime LIKE "%'.$_POST["search"]["value"].'%" ';

				$exam->query = $exam->query.' OR online_exam_duration LIKE "%'.$_POST["search"]["value"].'%" ';

				$exam->query = $exam->query.' OR total_question LIKE "%'.$_POST["search"]["value"].'%" ';

				$exam->query = $exam->query.' OR marks_per_right_answer LIKE "%'.$_POST["search"]["value"].'%" ';

				$exam->query = $exam->query.' OR marks_per_wrong_answer LIKE "%'.$_POST["search"]["value"].'%" ';

				$exam->query = $exam->query.' OR online_exam_status LIKE "%'.$_POST["search"]["value"].'%" ';
			}

			$exam->query = $exam->query.'';
			
			if(isset($_POST['order']))
			{
				$exam->query = $exam->query.' ORDER BY '.$_POST['order']['0']['column'].' '.$_POST['order']['0']['dir'].' ';
			}
			else
			{
				$exam->query = $exam->query.' ORDER BY online_exam_id DESC ';
			}
		...
		/*	if($_POST['length'] != -1)
			{
				$extra_query = $extra_query.'LIMIT ' . $_POST['start'] . ', ' . $_POST['length'];
			}*/
			
			$filtered_rows = $exam->total_row();
			
			$exam->query = $exam->query.$extra_query;
